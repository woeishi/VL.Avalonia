...
{
    /// <summary>
    /// Binds an <see cref="IChannel{TValue}"/> to an Avalonia property looked up by name on the input
    /// control, optionally converting between the channel value type and the property value type.
    /// </summary>
        ...
        where TControl : AvaloniaObject
    {
        ...
        {
            // The name resolves against the control's type, so a new control may resolve to a
            // different property - or resolve a name that was set before any input arrived.
            ResolveProperty();
        }
    ...
    /// <summary>
    /// A <see cref="BindingNameAdvancedNode{TControl, TValue, TProperty}"/> for the common case where
    /// the Avalonia property value type and the channel value type are the same.
    /// </summary>
    [ProcessNode(Name = "Binding (Name)")]
    public class BindingNameNode<TControl, TValue>
        : BindingNameAdvancedNode<TControl, TValue, TValue>
        where TControl : AvaloniaObject
    {
        [Fragment]
